...
class SQLService {
  ...
  Future addToCart(ShopItemModel data) async {
    await db?.transaction((txn) async {
      var qry =
          'INSERT INTO cart_list(shop_id, name, price, image,rating,fav,quantity) VALUES(${data.id}, "${data.name}",${data.price}, "${data.image}",${data.rating},${data.fav ? 1 : 0},1)';
      int id1 = await txn.rawInsert(qry);
      return id1;
    });
  }
  ...
  Future changeItemQuantity(int shop_id,int quantity ) async{
    var qry= "UPDATE cart_list SET quantity = $quantity WHERE shop_id = ${shop_id}";
